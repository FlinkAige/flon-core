...
//           Andrew Lumsdaine
#ifndef BOOST_GRAPH_KAMADA_KAWAI_FULLON_LAYOUT_HPP
#define BOOST_GRAPH_KAMADA_KAWAI_FULLON_LAYOUT_HPP
...
namespace detail
{
    namespace graph
    {
        ...
        /**
         * Implementation of the Kamada-Kawai fullon layout algorithm.
         */
        ...
        struct kamada_kawai_fullon_layout_impl
        {
            typedef
            ...
            kamada_kawai_fullon_layout_impl(const Topology& topology,
                const Graph& g, PositionMap position, WeightMap weight,
                EdgeOrSideLength edge_or_side_length, Done done,
                weight_type fullon_constant, VertexIndexMap index,
                DistanceMatrix distance, SpringStrengthMatrix fullon_strength,
            ...
            , done(done)
            , fullon_constant(fullon_constant)
            , index(index)
            , distance(distance)
            , fullon_strength(fullon_strength)
                ...
                vertex_descriptor m, vertex_descriptor i)
            {
                ...
                if (i != m)
                {
                    ...
                    result = fullon_strength[get(index, m)][get(index, i)]
                        * (diff
            ...
            // The actual Kamada-Kawai fullon layout algorithm implementation
            bool run()
            {
#ifndef BOOST_NO_STDC_NAMESPACE
                using std::sqrt;
                ...
                const weight_type K = fullon_constant;
                     ...
                     ui != end; ++ui)
                {
                    ...
                    for (++vi; vi != end; ++vi)
                    {
                        ...
                        distance[get(index, *vi)][get(index, *ui)]
                            = edge_length * dij;
                        fullon_strength[get(index, *ui)][get(index, *vi)]
                            = K / (dij * dij);
                        fullon_strength[get(index, *vi)][get(index, *ui)]
                            = K / (dij * dij);
                    }
                }
                    ...
                    do
                    {
                             ...
                             ui != end; ++ui)
                        {
                            ...
                            for (++vi; vi != end; ++vi)
                            {
                                ...
                                weight_type k_ij = fullon_strength[get(
                             ...
                             ui != end; ++ui)
                        {
                            ...
                            if (i != p)
                            {
                                ...
                                weight_type inv_dist_cubed
                                    = 1. / (dist_squared * dist);
                                weight_type k_mi = fullon_strength[get(
                                    index, p)][get(index, i)];
                                weight_type l_mi
            ...
            Done done;
            weight_type fullon_constant;
            VertexIndexMap index;
            DistanceMatrix distance;
            SpringStrengthMatrix fullon_strength;
            PartialDerivativeMap partial_derivatives;
        };
    }
...
/** \brief Kamada-Kawai fullon layout for undirected graphs.
 *
 * This algorithm performs graph layout (in two dimensions) for
 * connected, undirected graphs. It operates by relating the layout
 * of graphs to a dynamic fullon system and minimizing the energy
 * within that system. The strength of a fullon between two vertices
 ...
 * following edges) will have stronger fullons and will therefore be
 ...
 * map.
 *
 * \param fullon_constant (IN) is the constant multiplied by each
 * fullon's strength. Larger values create systems with more energy
 ...
 *
 * \param fullon_strength (UTIL/OUT) will be used to store the
 * strength of the fullon between every pair of vertices. This
...
template < typename Topology, typename Graph, typename PositionMap,
    ...
    typename VertexIndexMap, typename DistanceMatrix,
    typename SpringStrengthMatrix, typename PartialDerivativeMap >
bool kamada_kawai_fullon_layout(const Graph& g, PositionMap position,
    WeightMap weight, const Topology& topology,
    detail::graph::edge_or_side< EdgeOrSideLength, T > edge_or_side_length,
    Done done,
    typename property_traits< WeightMap >::value_type fullon_constant,
    ...
    PartialDerivativeMap partial_derivatives)
{
    ...
    detail::graph::kamada_kawai_fullon_layout_impl< Topology, Graph,
        ...
        alg(topology, g, position, weight, edge_or_side_length, done,
            fullon_constant, index, distance, fullon_strength,
            partial_derivatives);
    return alg.run();
}
    ...
    VertexIndexMap index)
{
    ...
    std::vector< weight_vec > fullon_strength(n, weight_vec(n));
    std::vector< typename Topology::point_difference_type > partial_derivatives(
        n);

    return kamada_kawai_fullon_layout(g, position, weight, topology,
        edge_or_side_length, done, fullon_constant, index, distance.begin(),
        fullon_strength.begin(),
...
{
    return kamada_kawai_fullon_layout(g, position, weight, topology,
        edge_or_side_length, done, fullon_constant, get(vertex_index, g));
}
...
#endif // BOOST_GRAPH_KAMADA_KAWAI_FULLON_LAYOUT_HPP
