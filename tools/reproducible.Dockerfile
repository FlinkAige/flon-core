...
ARG _FULLON_CLANG_VERSION=18.1.8
ARG _FULLON_LLVM_VERSION=11.1.0
ARG _FULLON_CMAKE_VERSION=3.27.6

ADD https://github.com/llvm/llvm-project/releases/download/llvmorg-${_FULLON_CLANG_VERSION}/llvm-project-${_FULLON_CLANG_VERSION}.src.tar.xz     \
    https://github.com/llvm/llvm-project/releases/download/llvmorg-${_FULLON_CLANG_VERSION}/llvm-project-${_FULLON_CLANG_VERSION}.src.tar.xz.sig \
    https://github.com/llvm/llvm-project/releases/download/llvmorg-${_FULLON_LLVM_VERSION}/llvm-project-${_FULLON_LLVM_VERSION}.src.tar.xz       \
    https://github.com/llvm/llvm-project/releases/download/llvmorg-${_FULLON_LLVM_VERSION}/llvm-project-${_FULLON_LLVM_VERSION}.src.tar.xz.sig   \
    https://github.com/Kitware/CMake/releases/download/v${_FULLON_CMAKE_VERSION}/cmake-${_FULLON_CMAKE_VERSION}.tar.gz                           \
    https://github.com/Kitware/CMake/releases/download/v${_FULLON_CMAKE_VERSION}/cmake-${_FULLON_CMAKE_VERSION}-SHA-256.txt                      \
    https://github.com/Kitware/CMake/releases/download/v${_FULLON_CMAKE_VERSION}/cmake-${_FULLON_CMAKE_VERSION}-SHA-256.txt.asc                  \
    /
...
RUN tar xf cmake-*.tar.gz && \
    cd cmake*[0-9] && \
    echo 'set(CMAKE_USE_OPENSSL OFF CACHE BOOL "" FORCE)' > fullon-init.cmake && \
    ./bootstrap --parallel=$(nproc) --init=fullon-init.cmake --generator=Ninja && \
    ninja install && \
    rm -rf cmake*

RUN tar xf llvm-project-${_FULLON_CLANG_VERSION}.src.tar.xz && \
    cmake -S llvm-project-${_FULLON_CLANG_VERSION}.src/llvm -B build-toolchain -GNinja -DLLVM_INCLUDE_DOCS=Off -DLLVM_TARGETS_TO_BUILD=host -DCMAKE_BUILD_TYPE=Release \
...
RUN tar xf llvm-project-${_FULLON_LLVM_VERSION}.src.tar.xz && \
    cmake -S llvm-project-${_FULLON_LLVM_VERSION}.src/llvm -B build-pinllvm -GNinja -DCMAKE_BUILD_TYPE=Release -DLLVM_TARGETS_TO_BUILD=host -DLLVM_BUILD_TOOLS=Off \
...
ARG FULLON_BUILD_JOBS

# Yuck: This places the source at the same location as fullon's CI (build.yaml, build_base.yaml). Unfortunately this location only matches
#       when build.yaml etc are being run from a repository named fullon.
COPY / /__w/fullon/fullon
RUN cmake -S /__w/fullon/fullon -B build -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=Release -GNinja && \
    cmake --build build -t package -- ${FULLON_BUILD_JOBS:+-j$FULLON_BUILD_JOBS} && \
    /__w/fullon/fullon/tools/tweak-deb.sh build/fullon_*.deb
