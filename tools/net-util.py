...
class netUtil:
    def __init__(self):
        self.prometheusMetrics = {
            ('funode_info', 'server_version'): 'funode Version ID:',
            ('funode_info', 'chain_id'): 'Chain ID:',
            ('funode_info', 'server_version_string'): 'funode Version:',
            ('funode_info', 'server_full_version_string'): 'funode Full Version:',
            ('funode_info', 'earliest_available_block_num'): 'Earliest Available Block:',
            'funode_head_block_num': 'Head Block Num:',
            'funode_last_irreversible': 'LIB:',
            'funode_p2p_clients': 'Inbound P2P Connections:',
            'funode_p2p_peers': 'Outbound P2P Connections:',
            'funode_blocks_incoming_total': 'Total Incoming Blocks:',
            'funode_trxs_incoming_total': 'Total Incoming Trxs:',
            'funode_blocks_produced_total': 'Blocks Produced:',
            'funode_trxs_produced_total': 'Trxs Produced:',
            'funode_scheduled_trxs_total': 'Scheduled Trxs:',
            'funode_unapplied_transactions_total': 'Unapplied Trxs:',
            'funode_p2p_dropped_trxs_total': 'Dropped Trxs:',
            'funode_p2p_failed_connections_total': 'Failed P2P Connections:',
            'funode_http_requests_total': 'HTTP Requests:',
        }
        self.ignoredPrometheusMetrics = [
            'funode_exposer_scrapes_total',
            'funode_exposer_transferred_bytes_total',
            'funode_subjective_bill_account_size_total',
            'funode_net_usage_us_total',
            'funode_cpu_usage_us_total',
        ]
        ...
        parser = argparse.ArgumentParser(description='Terminal UI for monitoring funode P2P connections',
        ...
        try:
            ...
            for family in text_string_to_metric_families(response.text):
                ...
                for sample in family.samples:
                    ...
                    elif sample.name == 'funode_p2p_addr':
                    ...
                    elif sample.name == 'funode_p2p_bytes_sent':
                        stats = bandwidths.get(connID, bandwidthStats())
                        stats.bytesSent = int(sample.value)
                        bandwidths[connID] = stats
                    elif fieldName == 'funode_p2p_block_sync_bytes_sent':
                        stats = bandwidths.get(connID, bandwidthStats())
                        stats.blockSyncBytesSent = int(sample.value)
                        bandwidths[connID] = stats
                    elif sample.name == 'funode_p2p_bytes_received':
                        stats = bandwidths.get(connID, bandwidthStats())
                        stats.bytesReceived = int(sample.value)
                        bandwidths[connID] = stats
                    elif sample.name == 'funode_p2p_connection_start_time':
                        stats = bandwidths.get(connID, bandwidthStats())
                        stats.connectionStarted = int(sample.value)
                        bandwidths[connID] = stats
                    elif sample.name == 'funode_p2p_connection_number':
                        pass
                    elif sample.name.startswith('funode_p2p_'):
                        fieldName = sample.name[len('funode_p2p_'):]
                    ...
                    if sample.name == 'funode_p2p_bytes_sent' or sample.name == 'funode_p2p_bytes_received' or sample.name == 'funode_p2p_block_sync_bytes_sent':
                        now = time.time_ns()
