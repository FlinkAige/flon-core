...
def readlogs(node_num, net_latency, funodeLogPath):
    filename = funodeLogPath
...
specificExtrafunodeArgs[producerNodeId]=" --p2p-listen-endpoint 0.0.0.0:{}".format(9876+producerNodeId)
specificExtrafunodeArgs[syncingNodeId]="--p2p-peer-address 0.0.0.0:{}".format(9876+producerNodeId)

try:
    TestHelper.printSystemInfo("BEGIN")
    tracefunodeArgs=" --plugin eosio::producer_plugin --produce-block-offset-ms 0 --plugin eosio::net_plugin --net-threads 1"
    if cluster.launch(pnodes=1, totalNodes=totalNodes, totalProducers=1, specificExtrafunodeArgs=specificExtrafunodeArgs, extrafunodeArgs=tracefunodeArgs) is False:
    ...
    testSuccessful=readlogs(syncingNodeId, latency, cluster.funodeLogPath)
    if platform.system() == 'Darwin':
