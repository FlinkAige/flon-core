...
#   error. The fork generated by funod should be sent to the ship_streamer so it is able to correctly observe the
#   fork.
#
###############################################################
...
try:
    ...
    specificExtrafunodArgs[shipNodeNum]="--plugin eosio::state_history_plugin --trace-history --chain-state-history --state-history-stride 200 --plugin eosio::net_api_plugin --plugin eosio::producer_api_plugin "
    if args.finality_data_history:
        specificExtrafunodArgs[shipNodeNum]+=" --finality-data-history"
    ...
    if cluster.launch(topo="./tests/bridge_for_fork_test_shape.json", pnodes=totalProducerNodes, loadSystemContract=False,
                      ...
                      extrafunodArgs=extrafunodArgs, specificExtrafunodArgs=specificExtrafunodArgs) is False:
    ...
    Print("Shutdown state_history_plugin funod after replay")
    shipNode.kill(signal.SIGTERM)
