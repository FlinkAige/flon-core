...
#   error. The fork generated by funode should be sent to the ship_streamer so it is able to correctly observe the
#   fork.
#
###############################################################
...
try:
    ...
    specificExtrafunodeArgs[shipNodeNum]="--plugin eosio::state_history_plugin --trace-history --chain-state-history --state-history-stride 200 --plugin eosio::net_api_plugin --plugin eosio::producer_api_plugin "
    if args.finality_data_history:
        specificExtrafunodeArgs[shipNodeNum]+=" --finality-data-history"
    ...
    if cluster.launch(topo="./tests/bridge_for_fork_test_shape.json", pnodes=totalProducerNodes, loadSystemContract=False,
                      ...
                      extrafunodeArgs=extrafunodeArgs, specificExtrafunodeArgs=specificExtrafunodeArgs) is False:
    ...
    Print("Shutdown state_history_plugin funode after replay")
    shipNode.kill(signal.SIGTERM)
