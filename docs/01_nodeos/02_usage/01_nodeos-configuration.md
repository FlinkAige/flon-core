---
content_title: funode Configuration
---

The plugin-specific options can be configured using either CLI options or a configuration file, `config.ini`. funode-specific options can only be configured from the command line. All CLI options and `config.ini` options can be found by running `funode --help` as shown above.
...
`~/.local/share/eosio/funode/config`

A custom `config.ini` file can be set by passing the `funode` option `--config path/to/config.ini`.

## funode Example

The example below shows a typical usage of `funode` when starting a block producing node:
...
The above `funode` command starts a producing node by:
...
* redirecting both `stdout` and `stderr` to the `funode.log` file
